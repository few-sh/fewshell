...
class MultiCommandApprovalOverlay extends ConsumerStatefulWidget {
  ...
  final ValueChanged<PendingToolCallList?>? onResolved;
  final bool embedded;
  ...
  static Future<PendingToolCallList?> show(
    ...
    final result = await showShadSheet<PendingToolCallList>(
      ...
      builder: (context) => MultiCommandApprovalOverlay(
        ...
        onResolved: (result) => Navigator.of(context).pop(result),
      ),
    );
  ...
  static Future<void> showTest(BuildContext context) async {
    ...
    final result = await show(context, pendingCalls);
    if (context.mounted) {
      if (result != null) {
...
class _MultiCommandApprovalOverlayState
  ...
  void _handleApprove() {
    ...
    ref.read(pendingToolCallApprovalProvider.notifier).approveSelected();
    widget.onResolved?.call(approvedCalls);
  }

  void _handleCancel() {
    ref.read(pendingToolCallApprovalProvider.notifier).cancelApproval();
    widget.onResolved?.call(const PendingToolCallList([]));
  }
  ...
  Widget build(BuildContext context) {
    ...
    final panelScrollContent = SingleChildScrollView(
              ...
              builder: (context, snapshot) {
                ...
                return ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final actionSecrets =
                        (call.arguments['secrets'] as List?)?.cast<String>() ??
                        [];
                    final allOptions = {
                      ...availableSecrets,
                      ...actionSecrets,
                                    ...
                                    children: [
                                      ...
                                      Expanded(
                                        child: ShadSelect<String>.multiple(
                                          ...
                                          closeOnSelect: false,
                                          initialValues: actionSecrets.toSet(),
                                          onChanged: (selected) {
                                            notifier.setSecrets(
                                              call.id,
                                              selected.toSet(),
    ...
    final constrainedPanelScrollContent = ConstrainedBox(
      constraints: BoxConstraints(
        maxHeight: MediaQuery.sizeOf(context).height * 0.35,
      ),
      child: panelScrollContent,
    );

    final panel = widget.embedded
        ? Container(
            decoration: BoxDecoration(
