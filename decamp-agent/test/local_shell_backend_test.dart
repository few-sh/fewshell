...
void main() {
  ...
  group('LocalShellBackend - Basic Execution', () {
    ...
    test('captures stderr correctly', () async {
      ...
      final result = await shellService.executeCommand('echo "error message" >&2');
    ...
    test('captures both stdout and stderr with mixed output script', () async {
      // Note: LocalShellBackend uses PTY which merges stderr into stdout
      final result = await shellService.executeCommand('bash $scriptsDir/mixed_output.sh');
    ...
    test('handles different exit codes', () async {
      ...
      final resultErr = await shellService.executeCommand('ls /nonexistent 2>&1 || true');
      expect(resultErr['stdout'], contains('No such file'));
    });

    test('handles command that does not exist', () async {
      final result = await shellService.executeCommand('nonexistent_command_xyz');
...
''';
    ...
    test('handles commands with pipes', () async {
      final result = await shellService.executeCommand('echo "hello world" | tr a-z A-Z');
    ...
    test('handles commands with redirections', () async {
      final tempFile = '/tmp/shell_test_${DateTime.now().millisecondsSinceEpoch}.txt';
  ...
  group('LocalShellBackend - Error Handling', () {
    ...
    test('handles syntax errors in bash', () async {
      final result = await shellService.executeCommand('if [ true then fi');
    ...
    test('handles command not found', () async {
      final result = await shellService.executeCommand('nonexistent_cmd_xyz_123');
      
      expect(result['stdout'], contains('not found'));
    });

    test('handles file not found', () async {
      final result = await shellService.executeCommand('cat /nonexistent/file/path.txt');
    ...
    test('handles permission denied', () async {
      ...
      final result = await shellService.executeCommand('cat /etc/shadow 2>&1 || echo "PERM_DENIED"');
  ...
  group('LocalShellBackend - Edge Cases', () {
    ...
    test('handles glob patterns', () async {
      final result = await shellService.executeCommand('ls /usr/bin/[a-c]* | head -5');
